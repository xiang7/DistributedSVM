...
int main(int argc, char **argv)
{
	...
	char *file_name = (char *) malloc(strlen(input_file_name)+2);
	sprintf(file_name, "%s%d", input_file_name, rank);
	char *cmd = (char *) malloc (1024);
	char *train_file = (char*) malloc(strlen(input_file_name)+3);
	sprintf(train_file, "%s.%d", input_file_name, rank);
	sprintf(cmd, "shuf %s | sed -n '1,%dp' > %s", input_file_name, lines/nproc, train_file);
	system(cmd);
